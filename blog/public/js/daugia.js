...
$(document).ready(function () {
    ...
    $('.btnAuction').click(function(){
        ...
        //gá»i the la vĂ o dc mĂ . Uh nhung ko biáº¿t POST data Ä‘Ăºng ko?
        $.ajax({ url: action,
            data: {
...
$(document).ready(function () {
    $('#stepAdd').click(function(){
        ...
        if(_iprice >= _total)
        {
            var _totalAdd = _total.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") + " Ä‘";
            $("#currentPrice").val(_totalAdd);
        }
        else
            alert("Lá»›n hÆ¡n giĂ¡ gá»‘c");
    });
    $('#stepSub').click(function(){
        ...
        {
            _subTotal = _subTotal.toString().replace(/(\d)(?=(\d{3})+(?!\d))/g, "$1.") + " Ä‘";
            $("#currentPrice").val(_subTotal);
        }
        else
            alert("BÆ°á»›c giĂ¡ nhá» hÆ¡n hiá»‡n táº¡i");
    });
});
$(document).ready(function () {
